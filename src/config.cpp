...
const char * richtext_default_script() {
	return
    "$font(,16)$cmd(Playback/Previous,Prev)$tab()$cmd(Playback/Next,Next)$tab()$cmd(Playback/Random,Random)\n"
    "$crlf(2)\n"
		"$font(,18)$if2(%title%,N/A)\n"
    "$crlf()\n"
		"$font(,16)<$if2(%artist%,N/A)>\n"
    "$crlf(2)\n"
    ...
    "$image(front)\n"
    "$crlf()\n"
    "$font(,18)$if2(%album%,N/A)\n"
    "$crlf()\n"
    "$font(,16)<$if2(%album artist%,N/A)>\n"
    "$crlf(2)\n"
    "<<$if2(%date%,)>>\n"
